...
module CodebreakerRackApp
  class SessionHelperSpec
    RSpec.describe SessionHelper do
      let(:session_helper) { SessionHelper }
      ...
      describe '.next_id' do
        it 'return "1" from initial' do
          expect(session_helper.next_id).to eq(1)
        end

        it 'return "111" when session_id = 110' do
          session_helper.class_variable_set :@@session_id, 110
          expect(session_helper.next_id).to eq(111)
        end
      end

      describe '.session' do
        before(:each) do
          allow(session_helper).to receive(:all).and_return(sessions)
        end
        it 'return third session when session_id = 3' do
          expect(session_helper.session(3).game).to eq(third_session.game)
        end
        it 'return first session when session_id = 1' do
          expect(session_helper.session(1).game).to eq(first_session.game)
        end
        it 'return nil when session_id = 4' do
          expect(session_helper.session(4)).to be_nil
        end
      end
      ...
      describe '.save' do
        ...
        context 'all has elements' do
          ...
          it 'call File.write array that contain pushed session' do
            ...
            session_helper.save(fourth_session)
          end
          ...
          it 'should update by given item' do
            ...
            session_helper.save(third_session)
          end
        end
      end
    end
